...
namespace PRoConEvents
{
	public class CRankKicker : PRoConPluginAPI, IPRoConPluginInterface
	{
		...
		{
			RegisterEvents(GetType().Name, "OnPlayerJoin", "OnListPlayers", "OnReservedSlotsPlayerAdded", "OnReservedSlotsPlayerRemoved", "OnReservedSlotsList", "OnReservedSlotsCleared");
		}
		...
		public override void OnListPlayers(List<CPlayerInfo> players, CPlayerSubset subset)
		{
			try
			{
				foreach (var player in players)
				{
					...
					if (rank == 0)
					{
						rank = GetGameTrackerBC2Stats(soldierName);
						if (rank == 0)
						{
							rank = GetBFBCStatRank(soldierName);
							if (rank == 0) continue;
						}
					}
		...
		private int GetGameTrackerBC2Stats(string soldierName)
		{
			...
			try
			{
				...
				{
					string url = "http://www.gametracker.com/games/bc2/stats/" + UrlEncode(soldierName) + "/";
		...
		private int GetBFBCStatRank(string soldierName)
		{
			try
			{
				string result;
				...
				using (var wc = new WebClient())
				{
					string url = "http://api.bfbcs.com/api/pc?players=" + UrlEncode(soldierName) + "&fields=smallinfo";
